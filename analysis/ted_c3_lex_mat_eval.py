"""Evaluate TED C3-Lex-Mat (LOO) outputs; write per-speaker metrics and CSV."""
import json
import re
...
INPUT = TED_PATH / "ted_c3_lex_mat_results.json"
METRICS_OUT = TED_PATH / "metrics" / "c3_lex_mat_metrics.json"
PER_SPEAKER_CSV = TED_PATH / "per_speaker" / "ted_per_speaker_c3_lex_mat.csv"
...
def main():
    ...
    for sample in results:
        ...
        hyp_raw = sample.get("c3_lex_mat_bm25_rag") or ""
        hyp = normalise(hyp_raw)
        if not hyp:
            print(f"SKIP {sample.get('speaker_id')} — empty C3-Lex-Mat output")
            continue
