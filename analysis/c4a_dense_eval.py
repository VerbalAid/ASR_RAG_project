...
with open('results/ted/c3_outputs.json', 'r') as f:  # c3_outputs = C4-Den-Gen (legacy)
    results = json.load(f)
...
for sample in results:
    ...
    c3 = normalise(sample['c3_rag_generic'] or '')
    if not c3:
...
print(f"\n--- C3 RAG GENERIC AVERAGES ---")
print(f"WER:       {avg('wer'):.4f}  (C1: 0.075 | C2: 0.387)")
print(f"BLEU:      {avg('bleu'):.4f}  (C1: 0.867 | C2: 0.482)")
print(f"ROUGE-L:   {avg('rouge_l'):.4f}  (C1: 0.943 | C2: 0.725)")
print(f"BERTScore: {avg('bert_score'):.4f}  (C1: 0.975 | C2: 0.925)")

with open('results/ted/metrics/c3_metrics.json', 'w') as f:
    json.dump(metrics_results, f, indent=2)
print("\nSaved to results/ted/metrics/c3_metrics.json")