...
\renewcommand{\abstractname}{Abstract\par\vspace{3}}
...
\noindent This paper evaluates retrieval-augmented generation (RAG) as a post-correction 
mechanism for ASR transcripts across three datasets, two retrieval modalities 
(BM25 vs dense embeddings), and three corpus domains. The central finding is a 
\emph{clinical corpus penalty}: domain-matched clinical retrieval significantly 
degrades biomedical entity fidelity (DISEASE F1, $p < 0.01$) while BERTScore 
remains unchanged, exposing standard semantic metrics as insufficient for 
clinical ASR evaluation. On clean speech, domain-matched dense RAG reduces WER 
by 54\% relative to an LLM-only baseline; on real clinical audio (PriMock57, 
$n$=57), it underperforms LLM-only on all four metrics ($p < 0.01$). 
Phonetically motivated ASR errors resist text-only correction, and loosely 
matched clinical corpora introduce incorrect entity substitutions invisible to 
semantic metrics. These findings point toward entity-aware retrieval, 
phonetic-based RAG, and graph-based approaches such as GraphRAG as necessary 
directions for safe clinical deployment.
...
The motivation for this project came from a practical frustration with long 
WhatsApp voice notes. Receiving a three-minute voice message to answer a yes/no 
question is a common experience, and it raises a straightforward question: can 
an automated system produce a concise, accurate text reformulation in the 
sender's own style? This is the longer-term applied direction of this work: a 
system for automatic voice message summarisation and reformulation. That vision 
has a clear prerequisite: the transcript has to be accurate first, since 
reformulating a noisy transcript would simply present incorrect content in 
fluent prose. This project therefore focuses on the correction step.

\medskip

The clinical domain raises a sharper version of the same problem. A drug name 
like \textit{lisinopril} transcribed as \textit{listen oh pill} can propagate 
into a clinical note and affect patient care, making entity-level fidelity a 
safety requirement. Critically, the danger is not only that errors go 
uncorrected: retrieval-based correction can actively introduce new errors, 
substituting entities that appear semantically plausible but are clinically 
wrong, in ways that standard metrics fail to detect.

\medskip

ASR models such as Whisper still fail under challenging conditions including 
accented speech, background noise, and domain-specific vocabulary. LLM-based 
post-correction is one remedy, but LLMs tend to paraphrase rather than minimally 
correct, harming lexical fidelity. RAG grounds the LLM in external passages and 
can improve correction quality, but corpus selection matters enormously: 
retrieving from a loosely related clinical corpus introduces medically plausible 
terminology that the model substitutes for uncertain ASR tokens, producing 
outputs that score well on semantic metrics while containing factually incorrect 
clinical entities. This failure mode is the central finding of the paper.

\medskip

Three research questions drive the design: (1)~Can RAG improve semantic quality 
over LLM-only rewriting? (2)~Does domain-specific retrieval outperform generic 
retrieval? (3)~How do BM25 and dense retrieval compare for ASR correction? These 
map to four testable hypotheses: (H1)~RAG improves semantic quality over 
LLM-only rewriting; (H2)~domain-specific retrieval outperforms generic 
retrieval; (H3)~dense retrieval outperforms lexical retrieval on identical 
corpora; and (H4)~RAG improves correction quality on real noisy clinical audio. 
Results are summarised in \S\ref{sec:hypotheses}. Nine conditions (C1 raw ASR, 
two LLM-only baselines, $2{\times}3$ RAG grid) are evaluated on WER, BLEU, 
ROUGE-L, BERTScore, and NER~F1.

\paragraph{Contributions.}
\begin{enumerate}[leftmargin=1.6em, itemsep=2pt, topsep=3pt]
  \item A controlled comparison of lexical and dense RAG pipelines across three 
  domains: clean TED, simulated clinical MTS-Dialog, and real clinical PriMock57.
  \item Identification and quantification of a \emph{clinical corpus penalty}: 
  domain-matched retrieval significantly degrades biomedical entity fidelity while 
  BERTScore stays flat.
  \item A contrast between simulated and real clinical noise showing why simulated 
  noise overestimates RAG's benefit.
  \item Evidence that safe clinical deployment requires entity-aware constraints, 
  with entity-aware retrieval, phonetic-based RAG, and GraphRAG identified as the 
...
\S\ref{sec:ted-sig}. Generic lexical retrieval (C3-Lex-Gen, BM25 over Wikipedia) is the worst corrected condition: per-speaker WER ranges from 0.244 to 0.724, reflecting retrieval luck rather than systematic correction.
...
$\ddagger$), a first sign of the clinical corpus penalty. The pattern is consistent across BM25 and dense conditions: it is corpus scope, not retrieval modality, that drives the degradation on clinical data.
...
The PriMock57 degradation is unlikely to be an artefact. Four considerations 
support this. First, the error type differs structurally from MTS-Dialog: 
MTS noise is randomly injected at the word level, leaving contextual signals 
intact so a language model can often reconstruct the intended word from 
surrounding context. PriMock57 errors are phonetically grounded --- Whisper 
has already transcribed a wrong word that is acoustically plausible, and the 
model receives only that wrong string with no acoustic signal to indicate it 
is incorrect. Text-context reconstruction is therefore fundamentally less 
tractable on phonetic errors than on random noise. Second, C3-Lex-Mat 
(WER 0.997) corroborates the finding from a different retrieval path: 
leave-one-out PriMock57 notes constitute the most tightly matched retrieval 
corpus available, yet performance is still worse than LLM-only --- if corpus 
mismatch alone were responsible, this condition should outperform C4-Den-Mat, 
but it does not. Third, the mechanism is coherent: retrieved clinical passages 
from other consultations introduce plausible-but-locally-incorrect drug names 
and diagnoses, which the model substitutes into the transcript. Fourth, the 
pipeline and evaluation are standard and no obvious implementation error would 
produce this pattern consistently across all four metrics.
...
entity fidelity. The hallucination case (Dialogue 434) is the most clinically serious failure. The patient explicitly stated no family history of major diseases, yet C4-Den-Mat inserted several sentences of fabricated neurological history, plausibly templated from a retrieved clinical note. This is not a paraphrase error --- it is factually incorrect clinical documentation introduced by retrieval. The entity-swap case (sarcoidosis for autoimmune disease) is 
subtler but equally problematic: both are disease entities so BERTScore 
registers little difference, but the two conditions are clinically distinct. 
The register-shift case (hypertension for high blood pressure) illustrates 
the opposite limitation: the substitution is clinically acceptable but 
penalised by exact-match NER because the entity string differs. No single 
metric captures clinical fidelity fully, which motivates entity-aware 
evaluation as a minimum requirement for clinical ASR assessment.
