"""
C3-Lex-Rel on TED: BM25 retrieval over the full TED ground-truth corpus (domain-relevant).

Uses ALL speakers' transcripts — no leave-one-out. (Leave-one-out = C3-Lex-Mat, see ted_c3_lex_mat.py.)
Chunking: 400 words, 50 overlap (same as C3-Lex-Gen).
Output: results/ted/ted_c3_lex_rel_results.json
"""
import json
import re
import time
...
OUTPUT_FILE = TED_PATH / "ted_c3_lex_rel_results.json"

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("c3_lex_rel_ted")
...
def build_bm25_full_corpus(results: list):
    """Build BM25 index from ALL TED ground-truth transcripts (no exclusion). C3-Lex-Rel = domain-relevant."""
    passages = []
    for r in results:
...
def main():
    ...
    for entry in results:
        ...
        passages, bm25 = build_bm25_full_corpus(results)
        log.info(f"  Corpus: {len(passages)} passages (full TED, no exclusion)")
        ...
        if ratio < 0.5:
            entry["c3_lex_rel_bm25_rag"] = None
            entry["c3_lex_rel_flag"] = "too_short"
        else:
            entry["c3_lex_rel_bm25_rag"] = final_output
            entry["c3_lex_rel_flag"] = "success"

        with OUTPUT_FILE.open("w") as f:
            json.dump(results, f, indent=2)

    log.info(f"C3b TED run complete. Results saved to {OUTPUT_FILE}")
